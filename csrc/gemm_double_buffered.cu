...
__global__ void gemm_double_buffered_kernel(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    *(float4*)&tileA[0][idx] = maskA_0 ? __ldcg((float4*)&A[(mt + idx / BLK_K) * K + (idx % BLK_K)]) : zero; // (m index) * K + (k index)
    ...
    for (int kt = 0; kt < K; kt += BLK_K) {
        ...
        for (int k = 0; k < BLK_K; k++) {
            ...
            for (int i = 0; i < FRAG_SIZE/2; i++) {
                fragA[i] = tileA[read][(tile_offset_m + i) * BLK_K + (k)];
                fragA[i + 4] = tileA[read][(tile_offset_m + i + WARP_TILE_H/2) * BLK_K + (k)];
        ...
        *(float4*)&tileA[write][idx] = nextA;
        *(float4*)&tileB[write][idx] = nextB;

        // swap read and write buffers
        read ^= 1; // ^= 1 flips 0 to 1 and vice versa
        write ^= 1;
...
torch::Tensor gemm_double_buffered(torch::Tensor A, torch::Tensor B) {
    ...
    gemm_double_buffered_kernel<<<grid, block>>>(t.A, t.B, t.C, t.M, t.N, t.K);
